    function resumeSlideBuilder(id){
        var picList = [];

        ajax2({
            url: "webAPIs/listResumeAPI.jsp",
            successFn: successResume,
            errorId: "error"
        });

        function successResume(obj){
            for (var i = 0; i < obj.resumeList.length; i++) {
                var pictures = {};
                pictures['image'] = obj.resumeList[i].image_url;
                pictures['email'] = obj.resumeList[i].resume_id;
            ...
            div.setAttribute("id", "resume-pics");
